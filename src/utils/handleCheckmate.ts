...
export function handleCheckmate(currentColor: "white" | "black") {
    ...
    const currentBoard: SquareOccupancy[][] = board.slice().map(row=>row.slice().map(sq => ({...sq})))
    const opponentPieces = currentBoard.flat().filter(sq => sq.pieceColor === opponentColor)!
    ...
    for (const piece of opponentPieces) {
        ...
        const selectedPiece = currentBoard.flat().find(sq => sq.x === currentX && sq.y === currentY)!
        selectedPiece.selected = true
        const possibleSquares: SquareOccupancy[][] = possibleMoves(currentBoard)
        const validMoves = possibleSquares.flat().filter(sq => sq.state === "possibleMove")
