...
function handlePawnForKing(selectedPiece: SquareOccupancy, possibleBoard: SquareOccupancy[][]) {
    possibleBoard;
    ...
    if (selectedPiece.pieceColor === 'white') {
        validMoves.push({ x: currentX - 1, y: currentY - 1, kill: false }, { x: currentX - 1, y: currentY + 1, kill: false })
    } else {
        validMoves.push({ x: currentX + 1, y: currentY - 1, kill: false }, { x: currentX + 1, y: currentY + 1, kill: false })
    }
    return validMoves;
...
function handleBishopForKing(selectedPiece: SquareOccupancy, possibleBoard: SquareOccupancy[][]) {
    const currentX = selectedPiece.x;
    const currentY = selectedPiece.y;
    const validMoves: validMoveInterface[] = [];
    const directions = [
    ...
    for (let i = 0; i < directions.length; i++) {
        ...
        while (true) {
            ...
            if (state === 'self' || state === 'kill' && possibleBoard[move.x][move.y].pieceType !== 'king') break;
...
function handleKnightForKing(selectedPiece: SquareOccupancy, possibleBoard: SquareOccupancy[][]) {
    possibleBoard;
    const validMoves: validMoveInterface[] = [];
    const currentX = selectedPiece.x;
    const currentY = selectedPiece.y;
...
function handleRookForKing(selectedPiece: SquareOccupancy, possibleBoard: SquareOccupancy[][]) {
    const currentX = selectedPiece.x;
    const currentY = selectedPiece.y;
    const validMoves: validMoveInterface[] = [];
    const directions = [
...
function handleQueenForKing(piece: SquareOccupancy, currentBoard: SquareOccupancy[][]) {
    const validMoves: validMoveInterface[] = [];
...
function handleKingForKing(selectedPiece: SquareOccupancy, possibleBoard: SquareOccupancy[][]) {
    possibleBoard;
    const validMoves: validMoveInterface[] = [];
    const currentX = selectedPiece.x;
    const currentY = selectedPiece.y;
