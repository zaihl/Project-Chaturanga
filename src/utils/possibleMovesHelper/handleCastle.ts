...
export function handleCastle(
  ...
  const myKing = currentBoard
    .flat()
    .find((sq) => sq.pieceColor === currentColor && sq.pieceType === "king")!;
  ...
  const historyIncludesKing = myHistory.some((move) =>
    move.piece.includes(`${currentColor}-king`),
  );
  if (historyIncludesKing || check) return validMoves;

  const historyIncludesKingSideRook = myHistory.some((move) =>
    move.piece.includes(`${currentColor}-rook-7`),
  );
  const historyIncludesQueenSideRook = myHistory.some((move) =>
    move.piece.includes(`${currentColor}-rook-0`),
  );
  ...
  const isKingSideClear = isWayClear(
    currentBoard,
    myKing.y + 1,
    myKing.y + 2,
    myKing.x,
  );
  const isQueenSideClear = isWayClear(
    currentBoard,
    myKing.y - 3,
    myKing.y - 1,
    myKing.x,
  ...
  const isKingSideChecked = isPositionChecked(
    currentBoard,
    { x: myKingX, y: kingSidePosition, kill: false },
    myKing,
  );
  const isQueenSideChecked = isPositionChecked(
    currentBoard,
    { x: myKingX, y: queenSidePosition, kill: false },
    myKing,
  );
...
function isPositionChecked(
  currentBoard: SquareOccupancy[][],
  validMoves: validMoveInterface,
