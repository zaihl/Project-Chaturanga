const Board = () => {
    ...
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
            ...
            const num = i * 8 + j
            boardArray.push(<Square key={num} lightColor={lightColor}>{num}</Square>)
        }
    }
...
const Square = (props: {children: React.ReactNode, lightColor: boolean}) => {
    return (
        <button className={`w-[calc(100vw/8-2px)] aspect-square xl:w-[5.25rem] ${props.lightColor ? "bg-green-100" : "bg-green-200"}`}>
            {props.children}
        </button>
    )
}
